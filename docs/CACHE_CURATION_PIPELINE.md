...
_Last revised: 2026-05-07. Scope: Arkadia desktop app (Avalonia 11 / .NET 8). This document describes the cache and curation pipeline as currently implemented, separating shipped behavior from planned future work._
...
Per-release dialog listing every known media asset for the release across all media types.

### What is shown

- File name, media type, on-disk size, status (`Preferred`, `Excluded`, `Missing`, or default), SHA-256 (when available), and a preview pane (image / video placeholder / PDF placeholder / generic).
- Status badge colors are applied for `Preferred` (green), `Excluded` (red), `Missing` (orange).

### What is not shown

Manage Media does not expose provider or source provenance. Where the asset came from (online ScreenScraper, cache, manual upload) is not surfaced in this dialog.

### Actions on a selected asset

- **Set Preferred** — enabled only when the asset exists, is not already preferred, and is not excluded.
- **Exclude** — disables the asset from auto-selection and from re-extraction (when "Respect excluded media" is on). Exclusions are currently recorded without a reason.
- **Restore** — clears an exclusion.
- **Save Credits** — persist a Credits string for the asset. Credits are curated attribution, not provider provenance.
- **Open File / Open Folder** — OS-level open / reveal-in-folder.
- **Delete File** — deletes the file from disk after confirmation. The curation record is retained as excluded so the asset is not silently reintroduced later.

### Add Media

Add Media is an embedded panel inside Manage Media. It:

- opens a system file picker with filters for images, video, PDF, and all files;
- prompts for target media type from `ReleaseMediaCurationService.MediaTypeOrder`;
- calls `AddMediaFile`, which copies the file into the canonical `data/media/<hardwareFamilyId>/<datLineId>/<media-type>/` location.

If every existing curation row for that media type is excluded, the newly added file is **not** auto-set as preferred — the user must explicitly Restore or Set Preferred.

There is no region selector. The picker is a system-level file selection only.
...
- **Auto-preferred is conservative.** New media added when no existing row is excluded for that type may be auto-set as preferred. If any row for that media type is excluded, the new file is not auto-promoted.
...
- **Expected baseline:** clean `incoming-csv/`, `scrape-cache/`, `staging-cache/` under the application directory.
...
1. Open Manage Media for a release with several assets. **Expected:** list, detail panel, preview, and badges render correctly.
2. **Set Preferred** on an asset. Pick another and set it Preferred. **Expected:** previous selection is no longer Preferred for that media type.
3. **Exclude** an asset. **Expected:** badge becomes `Excluded`; Restore is enabled, Set Preferred is disabled. (No reason prompt is shown.)
4. **Restore.** Expected: asset returns to default status.
5. **Save Credits** on an asset. **Expected:** persists across reload.
6. **Delete File** on an existing asset. **Expected:** file removed from disk; curation row remains as Excluded.
7. **Add Media** → file picker → choose target media type → confirm. **Expected:** file copied into the canonical media tree; if any existing row for that type is excluded, the new file is not auto-promoted to Preferred.
8. Confirm `physical-media` never appears in the UI; canonical type is `physical`.
...
**Status: planned. Not implemented.** No AMP/`.ark` code exists in the current build.
...
- **`.ark` extension and chunking** — planned package extension and chunked layout for large packages; only planned.
